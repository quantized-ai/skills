...
1. Diagnose each criterion with a brief task before explaining it.
2. If the student demonstrates the criterion, skip its instruction and routine practice.
3. Otherwise, teach only the missing idea, then provide varied practice with progressively less support.
4. During practice, identify the error before giving a hint. Demonstrate a different example only when hints are insufficient.
5. Assess with fresh tasks and no hints. Do not reuse worked examples with changed numbers.
6. Complete the lesson only when every criterion has independent mastery evidence. After a failed assessment, return to the affected criterion and reassess with a fresh task.
...
The student correctly classifies every required representation, identifies the decisive input when a relation is not a function, permits repeated outputs, and lists exact finite domain and range sets without duplicates or invented values.
