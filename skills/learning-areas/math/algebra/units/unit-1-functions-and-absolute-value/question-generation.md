...
5. Compare the candidate with the examples in the tutor file and bank and the student's available exposure record. Replace copied questions, rephrasings of the same problem, and repeated combinations of data and reasoning. Ask only after the candidate is valid and suitably fresh.

## Meaningful variation and difficulty

Vary coefficients, domain restrictions, endpoint inclusion, representation, context, direction of reasoning, and the kind of justification requested. Choose variations that remain inside the curriculum concept's scope and the tutor's delivery boundaries. For example, an absolute-value equation can ask for a solution set, a distance model, a solution-count explanation, or a diagnosis of missing cases. All can target the same criterion.
...
Use the curriculum path and exact Concept Title as the evidence key. The labels below match the operational headings in `tutor.md`; each heading links to its curriculum concept. These families guide creation, not a fixed sequence or an exhaustive list. Collect the tutor’s required Assess cases while retaining all curriculum proficiency requirements.

| Tutor section | Ways to generate distinct tasks | Checks before presentation |
| --- | --- | --- |
...
| Apply reflections and scale factors | Use a nonsymmetric complete point set; vary one multiplier; contrast visible and concealed reflections; reason about a zero multiplier. | Use reciprocal horizontal factors and signed coordinate mappings. For a zero outside multiplier and nonzero inside multiplier, retain the point mapping and original-domain restriction. With a zero inside multiplier, evaluate whether the parent is defined at zero; do not divide by zero. |
