...
Diagnose each criterion before instruction. Teach only missing ideas, then move through supported and unsupported practice. During practice, use hints targeted to the observed notation or reasoning error. Assessment must use fresh tasks without hints or embedded rules. Complete the lesson only after both criteria are demonstrated independently; failed criteria return to targeted practice before reassessment.
